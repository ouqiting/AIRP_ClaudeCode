...
- `python "{ROOT}/skills/wait_for_input.py"` — 实时输入监听（run_in_background 模式，阻塞等待用户输入）
...
- 读取卡片文件夹下的 `first_message.txt`（import_card.py 生成的开头选项）
...
```
然后等 2 秒让服务器就绪。
检查服务器是否已在运行：`curl -s http://localhost:8765/api/pending`
...
- **首条消息提取到 `./first_message.txt`**：从 `first_mes` 或 `alternate_greetings` 中提取全部开头，每个以 `=== 开头选项 N ===` 分隔
...
- 打印 JSON 摘要到 stdout 供 Claude Code 消费（含 `first_messages.count` 字段）
...
**`./first_message.txt`**（当前卡片文件夹）：import_card.py 自动生成，包含从角色卡提取的全部开头选项。每个选项以 `=== 开头选项 N (来源) ===` 分隔，后跟完整开头文本。

### 5. 加载历史
- 步骤 3.5 已加载 memory/ 下的记忆文件，结合 chat_log.json 重建完整叙事上下文
- 检查 `./chat_log.json`：
...
然后读取当前卡片文件夹下的 `first_message.txt`，根据导入脚本输出的 `first_messages.count` 判断：

**只有一个开头（count == 1）**：直接使用该开头作为开局，跳到步骤 8。

**有多个开头（count > 1）**：用 AskUserQuestion 询问用户选择哪个。每个选项显示该开头的**前 20 个字**作为简述，选项描述中提示「完整内容见 first_message.txt」。例如：
```
选项0: "北凛大学后街的便利店门口，自动门开合时带出..."（完整内容见 first_message.txt）
选项1: "北凛大学旧图书馆的旋转门发出年久失修的呻吟..."（完整内容见 first_message.txt）
```
用户选定后，从 `first_message.txt` 中提取对应开头的完整文本用于开局。


### 7. 开局
使用步骤 6 选定的开头文本作为开局内容。

**开局后，将内容写入 response.txt 并调用 handler：**
...
5. 启动后台输入监听：
   ...
   ###  后台输入监听（实时检测）
开局完成后，启动后台监听程序等待用户输入：
```
Bash: python "{ROOT}/skills/wait_for_input.py" (run_in_background, timeout=600000)
```
该程序每秒轮询 `/api/pending`，检测到用户输入后立即返回输入文本并退出。Claude Code 收到后台任务完成通知后立即处理。

**超时处理**：若后台任务超时（用户 10 分钟未回复），直接重新启动监听：
```
Bash: python "{ROOT}/skills/wait_for_input.py" (run_in_background, timeout=600000)
```
...
由 `wait_for_input.py` 后台任务实时触发（检测到用户输入后立即返回）。若后台任务超时，则重新启动监听。
